...
static const snand_flashdev_info gen_snand_FlashTable[] = {
	...
	{{0xC2, 0x26, 0x03}, 3, 256, 128, 2048, 128, 0x00000000, 0x00000000, 0x00000000,
		0x00000000, 0x0552000A, 0x3F00, 0x0000, "MX35LF2GE4AD", 0x00000000},
...
static bool mtk_snand_get_device_info(u8 *id, snand_flashdev_info *devinfo)
{
	...
	u32 target = -1;
	u8 target_id_len = 0;
	char buf_id[SNAND_MAX_ID * 3 + 1] = {0};
	...
	for (i = 0; i < ARRAY_SIZE(gen_snand_FlashTable); i++) {
		...
		if (mismatch == 0 &&
		    gen_snand_FlashTable[i].id_length > target_id_len) {
			target = i;
	...
	if (target != -1) {
		for (n = 0; n < gen_snand_FlashTable[target].id_length; n++) {
			...
			snprintf(buf_id + (n * 3), sizeof(buf_id) - (n * 3),
				"%02x ", devinfo->id[n]);
		}

		pr_info("SNAND: ID [ %s], [%s], Page[%d]B, Spare [%d]B Total [%d]MB\n",
			buf_id,
...
static void mtk_snand_select_chip(struct mtd_info *mtd, int chip)
{
	...
	if (chip == -1 && g_bInitDone == 0) {
		...
		if (spare_per_sector >= 28) {
			...
			pr_warn("[SNAND]: NFI not support oobsize: %x\n",
				spare_per_sector);
		}

		mtd->oobsize = spare_per_sector*(mtd->writesize/512);
		nand->ecc.strength = ecc_bit;
		pr_debug("[SNAND] select ecc bit: %d, sparesize: %d\n",
			ecc_bit, mtd->oobsize);
...
static int mtk_snand_probe(struct platform_device *pdev)
{
	...
	if (err) {
		dev_err(dev, "[mtk_snand] mtd parse partition error!\n");
		goto out_release;
	}
...
out:
	...
	kfree(g_snand_k_temp);
	g_snand_k_temp = NULL;

	dev_err(dev, "[mtk_snand] mtk_snand_probe fail, err = %d!\n", err);
