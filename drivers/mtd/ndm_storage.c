...
#define MTD_PART_STORAGE_1		"Storage_1"
#define MTD_PART_STORAGE_2		"Storage_2"
...
static int __init init_ndm_storage(void)
{
	...
	mtd = get_mtd_device_nm(MTD_PART_STORAGE_1);
	if (IS_ERR(mtd)) {
	...
	mtd = get_mtd_device_nm(MTD_PART_STORAGE_2);
	if (!IS_ERR(mtd)) {
