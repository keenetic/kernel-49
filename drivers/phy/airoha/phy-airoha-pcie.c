...
static void airoha_phy_clr_bits(void __iomem *reg, u32 mask)
{
...
static void airoha_phy_upd_bits(void __iomem *reg, u32 mask, u32 val)
{
	u32 tmp = readl(reg);
...
#define airoha_phy_upd_field(reg, mask, val)					\
	do {									\
		BUILD_BUG_ON_MSG(!__builtin_constant_p((mask)),			\
				 "mask is not constant");			\
		airoha_phy_upd_bits((reg), (mask),				\
				    FIELD_PREP((mask), (val)));			\
	} while (0)

#define airoha_phy_csr_2l_clr_bits(pcie_phy, reg, mask)				\
	airoha_phy_clr_bits((pcie_phy)->csr_2l + (reg), (mask))
#define airoha_phy_csr_2l_set_bits(pcie_phy, reg, mask)				\
	airoha_phy_set_bits((pcie_phy)->csr_2l + (reg), (mask))
#define airoha_phy_csr_2l_upd_field(pcie_phy, reg, mask, val)			\
	airoha_phy_upd_field((pcie_phy)->csr_2l + (reg), (mask), (val))

#define airoha_phy_pma0_clr_bits(pcie_phy, reg, mask)				\
	airoha_phy_clr_bits((pcie_phy)->pma0 + (reg), (mask))
#define airoha_phy_pma1_clr_bits(pcie_phy, reg, mask)				\
	airoha_phy_clr_bits((pcie_phy)->pma1 + (reg), (mask))
...
#define airoha_phy_pma0_upd_field(pcie_phy, reg, mask, val)			\
	airoha_phy_upd_field((pcie_phy)->pma0 + (reg), (mask), (val))
#define airoha_phy_pma1_upd_field(pcie_phy, reg, mask, val)			\
	airoha_phy_upd_field((pcie_phy)->pma1 + (reg), (mask), (val))

static void
airoha_phy_rx_fw_pre_cal(struct airoha_pcie_phy *pcie_phy, int lane,
			 enum airoha_pcie_port_gen gen)
{
	void __iomem *ofs = lane == 1 ? pcie_phy->pma1 : pcie_phy->pma0;
	u32 fl_out_target = gen == PCIE_PORT_GEN3 ? 41600 : 41941;
	u32 lock_cyclecnt = gen == PCIE_PORT_GEN3 ? 26000 : 32767;
	u32 csr_2l_reg, csr_2l_bit;
	u32 pr_idac, val, cdr_pr_idac_tmp = 0;
	int i;

	airoha_phy_set_bits(ofs + REG_PCIE_PMA_SS_LCPLL_PWCTL_SETTING_1,
			    PCIE_LCPLL_MAN_PWDB);
	airoha_phy_upd_field(ofs + REG_PCIE_PMA_SS_RX_FREQ_DET2,
			     PCIE_LOCK_TARGET_BEG, fl_out_target - 100);
	airoha_phy_upd_field(ofs + REG_PCIE_PMA_SS_RX_FREQ_DET2,
			     PCIE_LOCK_TARGET_END, fl_out_target + 100);
	airoha_phy_upd_field(ofs + REG_PCIE_PMA_SS_RX_FREQ_DET1,
			     PCIE_PLL_FT_LOCK_CYCLECNT, lock_cyclecnt);
	airoha_phy_upd_field(ofs + REG_PCIE_PMA_SS_RX_FREQ_DET4,
			     PCIE_LOCK_LOCKTH, 0x3);
	airoha_phy_upd_field(ofs + REG_PCIE_PMA_SS_RX_FREQ_DET3,
			     PCIE_UNLOCK_TARGET_BEG, fl_out_target - 100);
	airoha_phy_upd_field(ofs + REG_PCIE_PMA_SS_RX_FREQ_DET3,
			     PCIE_UNLOCK_TARGET_END, fl_out_target + 100);
	airoha_phy_upd_field(ofs + REG_PCIE_PMA_SS_RX_FREQ_DET1,
			     PCIE_PLL_FT_UNLOCK_CYCLECNT, lock_cyclecnt);
	airoha_phy_upd_field(ofs + REG_PCIE_PMA_SS_RX_FREQ_DET4,
			     PCIE_UNLOCK_LOCKTH, 0x3);

	if (lane == 1) {
		csr_2l_reg = REG_CSR_2L_CDR1_PR_INJ_MODE;
		csr_2l_bit = CSR_2L_PXP_CDR1_INJ_FORCE_OFF;
	} else {
		csr_2l_reg = REG_CSR_2L_CDR0_PR_INJ_MODE;
		csr_2l_bit = CSR_2L_PXP_CDR0_INJ_FORCE_OFF;
	}

	airoha_phy_csr_2l_set_bits(pcie_phy, csr_2l_reg, csr_2l_bit);

	airoha_phy_set_bits(ofs + REG_PCIE_PMA_FORCE_DA_PXP_CDR_PR_LPF_C,
			    PCIE_FORCE_SEL_DA_PXP_CDR_PR_LPF_R_EN);
	airoha_phy_set_bits(ofs + REG_PCIE_PMA_FORCE_DA_PXP_CDR_PR_LPF_C,
			    PCIE_FORCE_DA_PXP_CDR_PR_LPF_R_EN);
	airoha_phy_set_bits(ofs + REG_PCIE_PMA_FORCE_DA_PXP_CDR_PR_LPF_C,
			    PCIE_FORCE_SEL_DA_PXP_CDR_PR_LPF_C_EN);
	airoha_phy_clr_bits(ofs + REG_PCIE_PMA_FORCE_DA_PXP_CDR_PR_LPF_C,
			    PCIE_FORCE_DA_PXP_CDR_PR_LPF_C_EN);
	airoha_phy_set_bits(ofs + REG_PCIE_PMA_FORCE_DA_PXP_CDR_PR_IDAC,
			    PCIE_FORCE_SEL_DA_PXP_CDR_PR_IDAC);

	airoha_phy_set_bits(ofs + REG_PCIE_PMA_FORCE_DA_PXP_CDR_PR_PIEYE_PWDB,
			    PCIE_FORCE_SEL_DA_PXP_CDR_PR_PWDB);
	airoha_phy_clr_bits(ofs + REG_PCIE_PMA_FORCE_DA_PXP_CDR_PR_PIEYE_PWDB,
	...
	for (i = 0; i < LEQ_LEN_CTRL_MAX_VAL; i++) {
		pr_idac = (i + 1) << 8;
		airoha_phy_upd_field(ofs + REG_PCIE_PMA_FORCE_DA_PXP_CDR_PR_IDAC,
				     PCIE_FORCE_DA_PXP_CDR_PR_IDAC, pr_idac);
		airoha_phy_clr_bits(ofs + REG_PCIE_PMA_SS_RX_FREQ_DET4,
		...
		val = FIELD_GET(PCIE_RO_FL_OUT,
				readl(ofs + REG_PCIE_PMA_RO_RX_FREQDET));
	...
	airoha_phy_upd_field(ofs + REG_PCIE_PMA_FORCE_DA_PXP_CDR_PR_IDAC,
			     PCIE_FORCE_DA_PXP_CDR_PR_IDAC, cdr_pr_idac_tmp);
	...
	for (i = 0; i < FREQ_LOCK_MAX_ATTEMPT; i++) {
		...
		val = readl(ofs + REG_PCIE_PMA_RO_RX_FREQDET);
		if (val & PCIE_RO_FBCK_LOCK)
			break;
	}

	/* turn off force mode and update band values */
	airoha_phy_csr_2l_clr_bits(pcie_phy, csr_2l_reg, csr_2l_bit);
	...
	airoha_phy_clr_bits(ofs + REG_PCIE_PMA_FORCE_DA_PXP_CDR_PR_PIEYE_PWDB,
			    PCIE_FORCE_SEL_DA_PXP_CDR_PR_PWDB);
	airoha_phy_clr_bits(ofs + REG_PCIE_PMA_FORCE_DA_PXP_CDR_PR_IDAC,
			    PCIE_FORCE_SEL_DA_PXP_CDR_PR_IDAC);

	if (gen == PCIE_PORT_GEN3) {
		airoha_phy_upd_field(ofs + REG_PCIE_PMA_DIG_RESERVE_14,
				     PCIE_FLL_IDAC_PCIEG3, cdr_pr_idac_tmp);
	} else {
		airoha_phy_upd_field(ofs + REG_PCIE_PMA_SS_RX_FLL_1,
				     PCIE_RG_IPATH_IDAC, cdr_pr_idac_tmp);
		airoha_phy_upd_field(ofs + REG_PCIE_PMA_DIG_RESERVE_13,
				     PCIE_FLL_IDAC_PCIEG1, cdr_pr_idac_tmp);
		airoha_phy_upd_field(ofs + REG_PCIE_PMA_DIG_RESERVE_13,
				     PCIE_FLL_IDAC_PCIEG2, cdr_pr_idac_tmp);
	}
}

static void airoha_pcie_phy_init_default(struct airoha_pcie_phy *pcie_phy)
{
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_CMN,
...
{
	airoha_phy_csr_2l_upd_field(pcie_phy,
	...
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_CLKTX1_OFFSET,
				    CSR_2L_PXP_CLKTX1_OFFSET, 0x2);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_CLKTX0_FORCE_OUT1,
				   CSR_2L_PXP_CLKTX0_HZ);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_CLKTX1_OFFSET,
				   CSR_2L_PXP_CLKTX1_HZ);
	airoha_phy_csr_2l_upd_field(pcie_phy,
				    REG_CSR_2L_CLKTX0_FORCE_OUT1,
				    CSR_2L_PXP_CLKTX0_IMP_SEL, 0x12);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_CLKTX1_IMP_SEL,
				    CSR_2L_PXP_CLKTX1_IMP_SEL, 0x12);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_TXPLL_POSTDIV_D256,
				   CSR_2L_PXP_CLKTX0_SR);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_CLKTX1_OFFSET,
				   CSR_2L_PXP_CLKTX1_SR);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_PLL_CMN_RESERVE0,
...
static void airoha_pcie_phy_init_rx(struct airoha_pcie_phy *pcie_phy)
{
	...
	airoha_phy_csr_2l_clr_bits(pcie_phy,
	...
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_RX0_PHYCK_DIV,
				    CSR_2L_PXP_RX0_PHYCK_SEL, 0x1);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_RX1_PHYCK_DIV,
...
static void airoha_pcie_phy_init_jcpll(struct airoha_pcie_phy *pcie_phy)
{
	...
	airoha_phy_pma0_clr_bits(pcie_phy,
	...
	airoha_phy_pma1_set_bits(pcie_phy, REG_PCIE_PMA_FORCE_PXP_JCPLL_CKOUT,
				 PCIE_FORCE_SEL_DA_PXP_JCPLL_EN);
	airoha_phy_pma1_clr_bits(pcie_phy,
				 REG_PCIE_PMA_FORCE_PXP_JCPLL_CKOUT,
				 PCIE_FORCE_DA_PXP_JCPLL_EN);

	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_TCL_VTP_EN,
	...
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_JCPLL_SSC_PERIOD,
				   CSR_2L_PXP_JCPLL_SSC_PERIOD);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_JCPLL_SSC,
				   CSR_2L_PXP_JCPLL_SSC_PHASE_INI);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_JCPLL_SSC,
				   CSR_2L_PXP_JCPLL_SSC_TRI_EN);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_LPF_BR,
				    CSR_2L_PXP_JCPLL_LPF_BR, 0xa);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_LPF_BR,
				    CSR_2L_PXP_JCPLL_LPF_BP, 0xc);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_LPF_BR,
				    CSR_2L_PXP_JCPLL_LPF_BC, 0x1f);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_LPF_BWC,
				    CSR_2L_PXP_JCPLL_LPF_BWC, 0x1e);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_LPF_BR,
				    CSR_2L_PXP_JCPLL_LPF_BWR, 0xa);
	airoha_phy_csr_2l_upd_field(pcie_phy,
				    REG_CSR_2L_JCPLL_MMD_PREDIV_MODE,
				    CSR_2L_PXP_JCPLL_MMD_PREDIV_MODE, 0x1);
	airoha_phy_csr_2l_clr_bits(pcie_phy, CSR_2L_PXP_JCPLL_MONCK,
	...
	airoha_phy_pma1_set_bits(pcie_phy, REG_PCIE_PMA_FORCE_DA_PXP_RX_FE_VOS,
				 PCIE_FORCE_SEL_DA_PXP_JCPLL_SDM_PCW);
	airoha_phy_pma0_upd_field(pcie_phy,
				  REG_PCIE_PMA_FORCE_DA_PXP_JCPLL_SDM_PCW,
				  PCIE_FORCE_DA_PXP_JCPLL_SDM_PCW, 0x50000000);
	airoha_phy_pma1_upd_field(pcie_phy,
				  REG_PCIE_PMA_FORCE_DA_PXP_JCPLL_SDM_PCW,
				  PCIE_FORCE_DA_PXP_JCPLL_SDM_PCW, 0x50000000);
	...
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_RST_DLY,
				    CSR_2L_PXP_JCPLL_RST_DLY, 0x4);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_JCPLL_RST_DLY,
				   CSR_2L_PXP_JCPLL_SDM_DI_LS);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_JCPLL_TCL_KBAND_VREF,
				   CSR_2L_PXP_JCPLL_VCO_KBAND_MEAS_EN);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_JCPLL_IB_EXT,
				   CSR_2L_PXP_JCPLL_CHP_IOFST);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_IB_EXT,
	...
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_VCODIV,
	...
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_JCPLL_KBAND_KFC,
				   CSR_2L_PXP_JCPLL_KBAND_KFC);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_KBAND_KFC,
	...
	airoha_phy_pma0_clr_bits(pcie_phy, REG_PCIE_PMA_SCAN_MODE,
	...
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_SDM_HREN,
				    CSR_2L_PXP_JCPLL_TCL_AMP_GAIN, 0x1);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_SDM_HREN,
				    CSR_2L_PXP_JCPLL_TCL_AMP_VREF, 0x5);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_TCL_CMP,
				    CSR_2L_PXP_JCPLL_TCL_LPF_BW, 0x1);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_VCO_TCLVAR,
...
static void airoha_pcie_phy_txpll(struct airoha_pcie_phy *pcie_phy)
{
	...
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_TXPLL_SSC_PERIOD,
				   CSR_2L_PXP_txpll_SSC_PERIOD);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_TXPLL_CHP_IOFST,
				    CSR_2L_PXP_TXPLL_CHP_IOFST, 0x1);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_750M_SYS_CK,
				    CSR_2L_PXP_TXPLL_CHP_IBIAS, 0x2d);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_TXPLL_REFIN_DIV,
				   CSR_2L_PXP_TXPLL_REFIN_DIV);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_TXPLL_TCL_LPF_BW,
	...
	airoha_phy_pma0_upd_field(pcie_phy,
				  ...
				  PCIE_FORCE_DA_PXP_TXPLL_SDM_PCW, 0xc800000);
	airoha_phy_pma1_upd_field(pcie_phy,
				  REG_PCIE_PMA_FORCE_DA_PXP_TXPLL_SDM_PCW,
				  PCIE_FORCE_DA_PXP_TXPLL_SDM_PCW, 0xc800000);

	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_TXPLL_SDM_DI_LS,
				   CSR_2L_PXP_TXPLL_SDM_IFM);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_TXPLL_SSC,
				   CSR_2L_PXP_TXPLL_SSC_PHASE_INI);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_TXPLL_REFIN_DIV,
				    CSR_2L_PXP_TXPLL_RST_DLY, 0x4);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_TXPLL_SDM_DI_LS,
				   CSR_2L_PXP_TXPLL_SDM_DI_LS);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_TXPLL_SDM_DI_LS,
				    CSR_2L_PXP_TXPLL_SDM_ORD, 0x3);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_TXPLL_TCL_KBAND_VREF,
	...
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_750M_SYS_CK,
				    CSR_2L_PXP_TXPLL_CHP_IBIAS, 0x2d);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_TXPLL_TCL_VTP,
				    CSR_2L_PXP_TXPLL_SPARE_L, 0x1);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_TXPLL_LPF_BWR,
				   CSR_2L_PXP_TXPLL_LPF_BWC);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_TXPLL_POSTDIV,
	...
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_TXPLL_VCO_SCAPWR,
	...
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_TXPLL_TCL_KBAND_VREF,
				   CSR_2L_PXP_TXPLL_VCO_KBAND_MEAS_EN);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_TXPLL_VTP,
	...
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_750M_SYS_CK,
				   CSR_2L_PXP_TXPLL_LPF_SHCK_EN);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_TXPLL_POSTDIV,
				   CSR_2L_PXP_TXPLL_POSTDIV_EN);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_TXPLL_KBAND_DIV,
				   CSR_2L_PXP_TXPLL_KBAND_KFC);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_TXPLL_KBAND_DIV,
				    CSR_2L_PXP_TXPLL_KBAND_KF, 0x3);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_TXPLL_KBAND_DIV,
				    CSR_2L_PXP_txpll_KBAND_KS, 0x1);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_TXPLL_KBAND_DIV,
				    CSR_2L_PXP_TXPLL_KBAND_DIV, 0x4);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_TXPLL_LPF_BWR,
	...
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_TXPLL_SDM_OUT,
...
{
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_SSC_DELTA1,
				    CSR_2L_PXP_JCPLL_SSC_DELTA1, 0x106);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_SSC_DELTA1,
				    CSR_2L_PXP_JCPLL_SSC_DELTA, 0x106);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_JCPLL_SSC_PERIOD,
...
static void
airoha_pcie_phy_set_rxlane0_signal_detect(struct airoha_pcie_phy *pcie_phy)
{
	...
	airoha_phy_pma0_upd_field(pcie_phy, REG_PCIE_PMA_DIG_RESERVE_19,
				  PCIE_PCP_RX_REV0_PCIE_GEN1, 0x18b0);
	airoha_phy_pma0_upd_field(pcie_phy, REG_PCIE_PMA_DIG_RESERVE_20,
				  PCIE_PCP_RX_REV0_PCIE_GEN2, 0x18b0);
	airoha_phy_pma0_upd_field(pcie_phy, REG_PCIE_PMA_DIG_RESERVE_20,
				  PCIE_PCP_RX_REV0_PCIE_GEN3, 0x1030);

	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_RX0_SIGDET_DCTEST,
				    CSR_2L_PXP_RX0_SIGDET_PEAK, 0x2);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_RX0_SIGDET_VTH_SEL,
				    CSR_2L_PXP_RX0_SIGDET_VTH_SEL, 0x5);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_RX0_REV0,
				    CSR_2L_PXP_VOS_PNINV, 0x2);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_RX0_SIGDET_DCTEST,
				    CSR_2L_PXP_RX0_SIGDET_LPF_CTRL, 0x1);

	airoha_phy_pma0_upd_field(pcie_phy, REG_PCIE_PMA_SS_RX_CAL2,
				  PCIE_CAL_OUT_OS, 0x0);
	...
	airoha_phy_pma0_upd_field(pcie_phy, REG_PCIE_PMA_RX_FORCE_MODE0,
				  PCIE_FORCE_DA_XPON_RX_FE_GAIN_CTRL, 0x1);
	airoha_phy_pma0_upd_field(pcie_phy, REG_PCIE_PMA_SS_RX_SIGDET0,
				  PCIE_SIGDET_WIN_NONVLD_TIMES, 0x3);
	airoha_phy_pma0_clr_bits(pcie_phy, REG_PCIE_PMA_SEQUENCE_DISB_CTRL1,
...
static void
airoha_pcie_phy_set_rxlane1_signal_detect(struct airoha_pcie_phy *pcie_phy)
{
	...
	airoha_phy_pma1_upd_field(pcie_phy, REG_PCIE_PMA_DIG_RESERVE_19,
				  PCIE_PCP_RX_REV0_PCIE_GEN1, 0x18b0);
	airoha_phy_pma1_upd_field(pcie_phy, REG_PCIE_PMA_DIG_RESERVE_20,
				  PCIE_PCP_RX_REV0_PCIE_GEN2, 0x18b0);
	airoha_phy_pma1_upd_field(pcie_phy, REG_PCIE_PMA_DIG_RESERVE_20,
				  PCIE_PCP_RX_REV0_PCIE_GEN3, 0x1030);

	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_RX1_SIGDET_NOVTH,
				    CSR_2L_PXP_RX1_SIGDET_PEAK, 0x2);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_RX1_SIGDET_NOVTH,
				    CSR_2L_PXP_RX1_SIGDET_VTH_SEL, 0x5);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_RX1_REV0,
				    CSR_2L_PXP_VOS_PNINV, 0x2);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_RX1_DAC_RANGE_EYE,
				    CSR_2L_PXP_RX1_SIGDET_LPF_CTRL, 0x1);

	airoha_phy_pma1_upd_field(pcie_phy, REG_PCIE_PMA_SS_RX_CAL2,
				  PCIE_CAL_OUT_OS, 0x0);
	...
	airoha_phy_pma1_upd_field(pcie_phy,
				  REG_PCIE_PMA_FORCE_DA_PXP_RX_FE_GAIN_CTRL,
				  PCIE_FORCE_DA_PXP_RX_FE_GAIN_CTRL, 0x3);
	airoha_phy_pma1_upd_field(pcie_phy, REG_PCIE_PMA_RX_FORCE_MODE0,
				  PCIE_FORCE_DA_XPON_RX_FE_GAIN_CTRL, 0x1);
	airoha_phy_pma1_upd_field(pcie_phy, REG_PCIE_PMA_SS_RX_SIGDET0,
				  PCIE_SIGDET_WIN_NONVLD_TIMES, 0x3);
	airoha_phy_pma1_clr_bits(pcie_phy, REG_PCIE_PMA_SEQUENCE_DISB_CTRL1,
				 PCIE_DISB_RX_SDCAL_EN);
...
{
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_CDR0_PR_VREG_IBAND,
				    CSR_2L_PXP_CDR0_PR_VREG_IBAND, 0x5);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_CDR0_PR_VREG_IBAND,
				    CSR_2L_PXP_CDR0_PR_VREG_CKBUF, 0x5);

	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_CDR0_PR_CKREF_DIV,
				   CSR_2L_PXP_CDR0_PR_CKREF_DIV);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_CDR0_PR_COR_HBW,
	...
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_CDR1_PR_CKREF_DIV,
				   CSR_2L_PXP_CDR1_PR_CKREF_DIV);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_CDR1_PR_COR_HBW,
				   CSR_2L_PXP_CDR1_PR_CKREF_DIV1);

	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_CDR0_LPF_RATIO,
				    CSR_2L_PXP_CDR0_LPF_TOP_LIM, 0x20000);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_CDR1_LPF_RATIO,
				    CSR_2L_PXP_CDR1_LPF_TOP_LIM, 0x20000);

	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_CDR0_PR_BETA_DAC,
				    CSR_2L_PXP_CDR0_PR_BETA_SEL, 0x2);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_CDR1_PR_BETA_DAC,
...
static void airoha_pcie_phy_set_txflow(struct airoha_pcie_phy *pcie_phy)
{
	...
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_TX1_MULTLANE,
...
static void airoha_pcie_phy_set_rx_mode(struct airoha_pcie_phy *pcie_phy)
{
	...
	airoha_phy_pma0_upd_field(pcie_phy, REG_PCIE_PMA_DIG_RESERVE_18,
	...
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_CDR0_PR_MONCK,
				   CSR_2L_PXP_CDR0_PR_MONCK_ENABLE);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_CDR0_PR_MONCK,
	...
	airoha_phy_pma1_upd_field(pcie_phy, REG_PCIE_PMA_DIG_RESERVE_18,
				  PCIE_PXP_RX_VTH_SEL_PCIE_G1, 0x5);
	airoha_phy_pma1_upd_field(pcie_phy, REG_PCIE_PMA_DIG_RESERVE_18,
				  PCIE_PXP_RX_VTH_SEL_PCIE_G2, 0x5);
	airoha_phy_pma1_upd_field(pcie_phy, REG_PCIE_PMA_DIG_RESERVE_18,
	...
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_CDR1_PR_MONCK,
				   CSR_2L_PXP_CDR1_PR_MONCK_ENABLE);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_CDR1_PR_MONCK,
				    CSR_2L_PXP_CDR1_PR_RESERVE0, 0x2);
	airoha_phy_csr_2l_upd_field(pcie_phy, REG_CSR_2L_RX1_OSCAL_VGA1IOS,
...
static void airoha_phy_lane_reload(struct airoha_pcie_phy *pcie_phy, int lane)
{
	void __iomem *ofs = lane == 1 ? pcie_phy->pma1 : pcie_phy->pma0;

	airoha_phy_clr_bits(ofs + REG_PCIE_PMA_DIG_RESERVE_14,
			    PCIE_FLL_LOAD_BAND_EN);

	udelay(10);

	airoha_phy_set_bits(ofs + REG_PCIE_PMA_DIG_RESERVE_14,
			    PCIE_FLL_LOAD_BAND_EN);
}

static bool airoha_phy_lane_check(struct airoha_pcie_phy *pcie_phy, int lane)
{
	void __iomem *ofs = lane == 1 ? pcie_phy->pma1 : pcie_phy->pma0;
	u32 idacf, ipath;

	idacf = readl(ofs + REG_PCIE_PMA_DIG_RO_RESERVE_5);
	ipath = readl(ofs + REG_PCIE_PMA_SS_RX_FLL_1) & PCIE_RG_IPATH_IDAC;

	if (ipath == idacf)
		return true;

	airoha_phy_lane_reload(pcie_phy, lane);

	dev_warn(pcie_phy->dev, "lane%d ro_idacf not match (%x : %x), reload\n",
		 lane, idacf, ipath);
...
{
	/* workaround for gen1 auto K */
	airoha_phy_pma0_set_bits(pcie_phy, REG_PCIE_PMA_SS_RX_FLL_B,
				 PCIE_RG_LOAD_EN);
	airoha_phy_pma1_set_bits(pcie_phy, REG_PCIE_PMA_SS_RX_FLL_B,
				 PCIE_RG_LOAD_EN);
	airoha_phy_pma0_clr_bits(pcie_phy, REG_PCIE_PMA_DIG_RESERVE_14,
				 PCIE_FLL_LOAD_EN);
	airoha_phy_pma1_clr_bits(pcie_phy, REG_PCIE_PMA_DIG_RESERVE_14,
				 PCIE_FLL_LOAD_EN);

	airoha_phy_pma0_upd_field(pcie_phy, REG_PCIE_PMA_DIG_RESERVE_12,
				  PCIE_FORCE_PMA_RX_SPEED, 0xa);
	airoha_phy_pma1_upd_field(pcie_phy, REG_PCIE_PMA_DIG_RESERVE_12,
				  PCIE_FORCE_PMA_RX_SPEED, 0xa);

	airoha_phy_rx_fw_pre_cal(pcie_phy, 0, PCIE_PORT_GEN3);
	airoha_phy_rx_fw_pre_cal(pcie_phy, 1, PCIE_PORT_GEN3);

	airoha_phy_pma0_clr_bits(pcie_phy, REG_PCIE_PMA_DIG_RESERVE_12,
				 PCIE_FORCE_PMA_RX_SPEED);
	airoha_phy_pma1_clr_bits(pcie_phy, REG_PCIE_PMA_DIG_RESERVE_12,
				 PCIE_FORCE_PMA_RX_SPEED);

	usleep_range(100, 200);

	airoha_phy_rx_fw_pre_cal(pcie_phy, 0, PCIE_PORT_GEN2);
	airoha_phy_rx_fw_pre_cal(pcie_phy, 1, PCIE_PORT_GEN2);

	/* workaround for gen1 auto K */
	airoha_phy_lane_reload(pcie_phy, 0);
	airoha_phy_lane_reload(pcie_phy, 1);
}
...
static int airoha_pcie_phy_init(struct phy *phy)
{
	...
	/* Rx lane0 signal detect */
	airoha_pcie_phy_set_rxlane0_signal_detect(pcie_phy);

	/* Rx lane1 signal detect */
	airoha_pcie_phy_set_rxlane1_signal_detect(pcie_phy);
	...
	airoha_phy_pma0_clr_bits(pcie_phy, REG_PCIE_PMA_SS_DA_XPON_PWDB0,
				 PCIE_DA_XPON_CDR_PR_PWDB);
	airoha_phy_pma1_clr_bits(pcie_phy, REG_PCIE_PMA_SS_DA_XPON_PWDB0,
				 PCIE_DA_XPON_CDR_PR_PWDB);
	...
	/* check lane ro_idacf, reset CDR if idacf value not match */
	for (lane = 0; lane < 2; lane++) {
		int retry;

		for (retry = 0; retry < 5; retry++) {
			if (airoha_phy_lane_check(pcie_phy, lane))
				break;
		}
	}
...
static int airoha_pcie_phy_exit(struct phy *phy)
{
	...
	airoha_phy_pma0_clr_bits(pcie_phy, REG_PCIE_PMA_SW_RESET,
				 PCIE_PMA_SW_RST);
	airoha_phy_pma1_clr_bits(pcie_phy, REG_PCIE_PMA_SW_RESET,
				 PCIE_PMA_SW_RST);
	airoha_phy_csr_2l_clr_bits(pcie_phy, REG_CSR_2L_JCPLL_SSC,
				   CSR_2L_PXP_JCPLL_SSC_PHASE_INI |
				   CSR_2L_PXP_JCPLL_SSC_TRI_EN |
...
static int airoha_pcie_phy_probe(struct platform_device *pdev)
{
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "csr-2l");
	pcie_phy->csr_2l = devm_ioremap_resource(dev, res);
	if (IS_ERR(pcie_phy->csr_2l)) {
		dev_err(dev, "Failed to map %s base\n", "csr-2l");
		return PTR_ERR(pcie_phy->csr_2l);
	}

	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "pma0");
	pcie_phy->pma0 = devm_ioremap_resource(dev, res);
	if (IS_ERR(pcie_phy->pma0)) {
		dev_err(dev, "Failed to map %s base\n", "pma0");
		return PTR_ERR(pcie_phy->pma0);
	}

	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "pma1");
	pcie_phy->pma1 = devm_ioremap_resource(dev, res);
	if (IS_ERR(pcie_phy->pma1)) {
		dev_err(dev, "Failed to map %s base\n", "pma1");
		return PTR_ERR(pcie_phy->pma1);
	}

	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "p0-xr-dtime");
	pcie_phy->p0_xr_dtime = devm_ioremap_resource(dev, res);
	if (IS_ERR(pcie_phy->p0_xr_dtime)) {
		dev_err(dev, "Failed to map %s base\n", "p0-xr-dtime");
		return PTR_ERR(pcie_phy->p0_xr_dtime);
	}

	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "p1-xr-dtime");
	pcie_phy->p1_xr_dtime = devm_ioremap_resource(dev, res);
	if (IS_ERR(pcie_phy->p1_xr_dtime)) {
		dev_err(dev, "Failed to map %s base\n", "p1-xr-dtime");
		return PTR_ERR(pcie_phy->p1_xr_dtime);
	}

	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "rx-aeq");
	pcie_phy->rx_aeq = devm_ioremap_resource(dev, res);
	if (IS_ERR(pcie_phy->rx_aeq)) {
		dev_err(dev, "Failed to map %s base\n", "rx-aeq");
		return PTR_ERR(pcie_phy->rx_aeq);
	}
...
static const struct of_device_id airoha_pcie_phy_of_match[] = {
	{ .compatible = "airoha,an7581-pcie-phy" },
	{ /* sentinel */ }
};
...
MODULE_DESCRIPTION("Airoha PCIe Gen3 PHY driver");
