			 ...
			 const char *		extra)
{
	...
	if (descr->header_type == IW_HEADER_TYPE_POINT) {
		...
		if (wrqu->data.length > descr->max_tokens) {
			netdev_err(dev, "(WE) : Wireless Event (cmd=0x%04X, flags=0x%04X) too big (%d)\n",
				   cmd, wrqu->data.flags, wrqu->data.length);
			return;
		}
		if (wrqu->data.length < descr->min_tokens) {
			netdev_err(dev, "(WE) : Wireless Event (cmd=0x%04X, flags=0x%04X) too small (%d)\n",
				   cmd, wrqu->data.flags, wrqu->data.length);
			return;
		}
